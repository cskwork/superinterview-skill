...
Dispatch is the default: the conductor runs each role as a separate fresh-context subagent so the role's
references load inside the subagent and never accumulate in the conductor's window. The subagent returns
only a short structured result - the posed question, the scorecard, the gap, the re-drill - not its
transcript. A single quick GRADE of one short answer runs inline. Full personas: `agents/interviewer.md`,
`agents/critic.md`.
